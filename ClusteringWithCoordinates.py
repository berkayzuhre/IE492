...
# line_coordinates=pd.read_excel('line_coordinates.xlsx')
...
for i in range(1,len(kmeans.labels_)):
	#sheet.cell(row=i, column=1, value=line_coordinates_numpy.index.values[i-1])
	sheet.cell(row=i, column=1, value=RequirementScores.columns.values[i-1])
	sheet.cell(row=i, column=2, value=kmeans.labels_[i-1])

#workbook.save(filename="clusters_with_mean_coordinates.xlsx")
workbook.save(filename="clusters_with_reqscores_coordinates.xlsx")